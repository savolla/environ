...
let
  ...
  HOSTNAME = "xkarna";
  HOME = "/home/savolla";
  ETHERNET_INTERFACE_NAME = "enp4s0";
  WIRELESS_INTERFACE_NAME = "wlp2s0";
  STATIC_IP_ADDRESS = "192.168.1.250";
...
in {
  ...
  # auto mount my 1TB Samsung T7 Shield SSD that contains my home folder /home/savolla
  boot.initrd.luks.devices."savolla".device =
    "/dev/disk/by-uuid/5ba30f2e-b06a-4588-b594-70fb46ef16d9";
  ...
  # nix config
  nix = {
    # gc = { # commented because I do this with nixos-update and nixos-rebuild scripts. taking care of my ssd
    #   automatic = true;
    #   dates = "daily";
    ...
    settings = {
      ...
      # auto-optimise-store = true; # commented because I already do this manually with nixos-rebuild. taking care of my ssd
    };
  };
  ...
  environment.systemPackages = with pkgs; [
    ...
    freetube # youtube blocks my video stream after 1 minute. so I installed this
    ...
    # org-mode
    texliveFull # org-mode pdf export and other goodies
    (python311.withPackages (ps: with ps; [ jupyter ])) # required by +jupyter
    ...
    (pkgs.buildFHSEnv {
      ...
      targetPkgs = pkgs:
        with pkgs; [
          ...
          # 32-bit libraries (for 32-bit Wine)
  ...
  services = {
    ...
    spice-vdagentd = { # share clipboard, files etc. with virtual machines
      enable = true;
    };
  };
    ...
    musician = {
      ...
      configuration = {
        environment.variables = {
          CURRENT_NIXOS_SPECIALISATION = lib.mkForce "musician";
        };
        imports = [ /home/savolla/resource/repos/musnix ];
        musnix.enable = true;
      };
    };
